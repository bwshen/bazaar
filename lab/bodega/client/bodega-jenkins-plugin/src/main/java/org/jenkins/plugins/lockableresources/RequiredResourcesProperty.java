...
public class RequiredResourcesProperty extends JobProperty<Job<?, ?>> {
    ...
    public String getBodegaItemRequirements() {
        return bodegaItemRequirements;
    }
	...
	public static class DescriptorImpl extends JobPropertyDescriptor {
		...
		public RequiredResourcesProperty newInstance(StaplerRequest req,
			...
			String bodegaItemRequirements = Util.fixEmptyAndTrim(json
					.getString("bodegaItemRequirements"));
			...
			return new RequiredResourcesProperty(
                            ...
                            labelName, bodegaItemRequirements);
		}

        /* Jenkins Pixie Dust -- A public method that starts with doCheck is
         * invoked as part of form validation. the first oarameter is passed
         * as a string of the form field in question. All other fields that
         * need to be used for validation are optional and need to be
         * explicitly passed*/
		public FormValidation doCheckBodegaResourceSpecs(
                @QueryParameter String value,
                @QueryParameter String resourceNames,
                @QueryParameter String labelName,
                @QueryParameter String resourceNumber) {
            String specs = Util.fixEmptyAndTrim(value);
            if (specs == null) {
                return FormValidation.ok();
            }
            String names = Util.fixEmptyAndTrim(resourceNames);
            if (names != null)
                return FormValidation.error("Specs and Resource Names Can not co-exist");

            String labels = Util.fixEmptyAndTrim(labelName);
            if (labels != null)
                return FormValidation.error("Specs and labels can not co-exist");

            String resourceNumbers = Util.fixEmptyAndTrim(resourceNumber);
            if (resourceNumbers != null)
                return FormValidation.error("Resource count not allowed");
